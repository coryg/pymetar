import re
import datetime
from .datatypes import Temperature, BarometricPressure, WindSpeed, Distance, \
...
class METARObservation(object):
    ...
    def __init__(self, metarcode, month=None, year=None, utcdelta=None):
        ...
        try:
            ngroup = len(METARObservation.handlers)
            igroup = 0
            ifailed = -1

            while igroup < ngroup and code:
                pattern, handler, repeatable = METARObservation.handlers[igroup]
                if debug:
            ...
            if pattern == REMARK_RE or self.press:
                while code:
                    for pattern, handler in METARObservation.remark_handlers:
                        if debug:
                            print(handler.__name__, ":", code)
                        m = pattern.match(code)
                        if m:
    ...
    def _do_trend_handlers(self, code):
        for pattern, handler, repeatable in METARObservation.trend_handlers:
            if debug:
                print(handler.__name__, ":", code)
            m = pattern.match(code)
            while m:
    ...
    def _handleWind(self, d):
        ...
        if wind_speed.startswith("P"):
            self.wind_speed = WindSpeed(wind_speed[1:], units, ">")
        elif not MISSING_RE.match(wind_speed):
            self.wind_speed = WindSpeed(wind_speed, units)
        if d['gust']:
            wind_gust = d['gust']
            if wind_gust.startswith("P"):
                self.wind_gust = WindSpeed(wind_gust[1:], units, ">")
            elif not MISSING_RE.match(wind_gust):
                self.wind_gust = WindSpeed(wind_gust, units)
        if d['varfrom']:
        ...
        if press != '////':
            ...
            if d['unit']:
                if d['unit'] == 'A' or (d['unit2'] and d['unit2'] == 'INS'):
                    self.press = BarometricPressure(press / 100, 'IN')
                elif d['unit'] == 'SLP':
                    if press < 500:
                        press = press / 10 + 1000
                    else:
                        press = press / 10 + 900
                    self.press = BarometricPressure(press, 'MB')
    ...
    def _handleSealvlPressRemark(self, d):
        ...
        if value < 50:
            ...
            value += 900
        if not self.press:
            self.press = BarometricPressure(value, "MB")
        self.press_sea_level = BarometricPressure(value, "MB")
    ...
    def _handlePeakWindRemark(self, d):
        ...
        self.wind_speed_peak = WindSpeed(peak_speed, "KT")
