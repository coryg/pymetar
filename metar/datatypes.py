import re
from math import sin, cos, atan2, sqrt, atan, pi
...
class WindSpeed(object):
    ...
    def __init__(self, value, units=None, gtlt=None):
        if not units:
            ...
            if not units.upper() in WindSpeed.legal_units:
                raise UnitsError("unrecognized speed unit: '" + units + "'")
            self._units = units.upper()
        if gtlt and not gtlt in WindSpeed.legal_gtlt:
